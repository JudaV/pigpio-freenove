...
def setup():
    ...
    iets = random.randint(1,9)
    p_Red = GPIO.PWM(ledRedPin,2)    # configure PMW for RGBLED pins, set PWM Frequence to 1kHz
    p_Red.start(0)
    p_Green = GPIO.PWM(ledGreenPin,5)
    p_Green.start(0)
    p_Blue = GPIO.PWM(ledBluePin,3)
    p_Blue.start(0)
    #p_Blue = GPIO.PWM(ledBluePin,5)
...
def loop():
    while True:
        ...
        #p_Blue = GPIO.PWM(ledBluePin,iets)
        #p_Blue.start(0)
        time.sleep(1)
