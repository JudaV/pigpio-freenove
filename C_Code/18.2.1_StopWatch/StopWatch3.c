/*
 * Filename:    StopWatch3.c
 * Project:     Freenove kit using pigpio C and Python library
 * Description: use of 74HC595 shift register with 4-digit 7-segment display
 * Author:      JudaV
 * date:        october 2024
 * compile:     gcc -o StopWatch3 StopWatch3.c -lpigpio -lpthread
 *              or with makefile:  make
 * usage:       sudo ./StopWatch3
 */
...
{ // Timer function
    if (sig == SIGALRM)
    { // If the signal is SIGALRM, the value of counter plus 1, and update the number displayed by 7-segment display
        counter++;
